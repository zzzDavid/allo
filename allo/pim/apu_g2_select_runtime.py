...
"""Hardware-only uint16 compare/select primitive for Gemini-II."""
...
from .apu_g2_recipe import build_apu_g2_u16_select_lt_recipe
...
def _operand(value, name: str) -> np.ndarray:
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane select gate")
    return ticks, host


def run_apu_g2_u16_select_lt(
    lhs,
    rhs,
    true_value,
    false_value,
    *,
    ...
    """Run ``lhs < rhs ? true_value : false_value`` on the real APUg2 card."""

    lhs = _operand(lhs, "lhs")
    rhs = _operand(rhs, "rhs")
    true_value = _operand(true_value, "true_value")
    false_value = _operand(false_value, "false_value")
    ...
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-select-lt-"))
    ...
    try:
        ...
        true_path = root / "true.bin"
        false_path = root / "false.bin"
        output_path = root / "out.bin"
        lhs.tofile(lhs_path)
        rhs.tofile(rhs_path)
        true_value.tofile(true_path)
        false_value.tofile(false_path)
        ...
        host_binary = host_build / "tenon_apu_g2_u16_select_lt"
        ...
        if not np.array_equal(output, expected):
            ...
            raise RuntimeError(
                f"APUg2 select result differs from NumPy at {index}: "
...
__all__ = ["run_apu_g2_u16_select_lt"]
