...
"""Hardware-only uint16 logical right-shift primitive for Gemini-II."""
...
from .apu_g2_recipe import build_apu_g2_u16_shift_right_recipe
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane shift gate")
    return ticks, host


def run_apu_g2_u16_shift_right(src, shift, *, repetitions=128) -> RunResult:
    """Run logical ``src >> shift`` on the real APUg2 card."""
    ...
    recipe = build_apu_g2_u16_shift_right_recipe(shift)

    keep = os.environ.get(_KEEP_ENV) == "1"
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-shift-right-"))
    ...
    try:
        ...
        host_binary = host_build / "tenon_apu_g2_u16_shift_right"
        ...
        if not np.array_equal(output, expected):
            ...
            raise RuntimeError(
                f"APUg2 shift_right result differs from NumPy at {index}: "
...
__all__ = ["run_apu_g2_u16_shift_right"]
