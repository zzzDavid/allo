...
"""Hardware-only uint16 modular multiplication primitive for Gemini-II."""
...
from .apu_g2_recipe import build_apu_g2_u16_mul_recipe
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane mul gate")
    return ticks, host


def run_apu_g2_u16_mul(lhs, rhs, *, repetitions=64) -> RunResult:
    """Run ``lhs * rhs mod 2^16`` on the real APUg2 card."""
    ...
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-mul-"))
    ...
    try:
        ...
        host_binary = host_build / "tenon_apu_g2_u16_mul"
        ...
        if not np.array_equal(output, expected):
            ...
            raise RuntimeError(
                f"APUg2 mul result differs from NumPy at {index}: "
