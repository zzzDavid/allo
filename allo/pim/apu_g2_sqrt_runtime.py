...
"""Hardware-only uint16 floor-square-root primitive for Gemini-II."""
...
def sqrt_u16_reference(src: np.ndarray) -> np.ndarray:
    """Reference ``floor(sqrt(src))`` for a full uint16 vector pack."""

    src = _validate_operand("src", src)
    # uint16 values fit exactly in float64; casting back after floor is exact.
    return np.floor(np.sqrt(src.astype(np.float64))).astype(np.uint16)
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane sqrt gate")
    return ticks, host


def run_apu_g2_u16_sqrt(src, *, repetitions=16) -> RunResult:
    """Run ``floor(sqrt(src))`` over one full ``(4, 65536)`` uint16 pack."""
    ...
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-sqrt-"))
    ...
    try:
        ...
        host_binary = host_build / "tenon_apu_g2_u16_sqrt"
        ...
        if output.size != math.prod(_SHAPE):
            raise RuntimeError(
                f"hardware output has {output.size} elements, expected {math.prod(_SHAPE)}"
            )
        ...
        if not np.array_equal(output, expected):
            ...
            raise RuntimeError(
                f"APUg2 sqrt result differs from NumPy at {index}: "
...
__all__ = ["run_apu_g2_u16_sqrt", "sqrt_u16_reference"]
