...
"""Hardware-only uint16 subtraction primitive for Gemini-II."""
...
from .apu_g2_recipe import build_apu_g2_u16_sub_recipe
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane sub gate")
    return ticks, host


def run_apu_g2_u16_sub(lhs, rhs, *, repetitions=128) -> RunResult:
    """Run unsigned modulo-2^16 ``lhs - rhs`` on the real APUg2 card."""
    ...
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-sub-"))
    ...
    try:
        ...
        host_binary = host_build / "tenon_apu_g2_u16_sub"
        ...
        if not np.array_equal(output, expected):
            ...
            raise RuntimeError(
                f"APUg2 sub result differs from NumPy at {index}: "
