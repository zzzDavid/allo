...
constexpr uint32_t kOutRow = 512;
    ...
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
        ...
        const uint16_t expected = static_cast<uint16_t>(
            static_cast<uint32_t>(lhs[offset]) + rhs[offset]);
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    try {
      ...
      const ApuG2AddParams params{kLhsRow, kRhsRow, kOutRow, repetitions,
                                  timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_add", params);
      ...
      std::cout << "device_input_copy_ticks=" << timings.input_copy_ticks
                << '\n';
      std::cout << "device_add_ticks=" << timings.add_ticks << '\n';
      std::cout << "device_output_copy_ticks=" << timings.output_copy_ticks
                << '\n';
    ...
    std::cerr << "APUg2 host failure: " << error.what() << '\n';
    return 1;
  }
}
