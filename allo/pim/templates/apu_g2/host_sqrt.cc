...
constexpr uint32_t kOutRow = 256;
...
uint16_t isqrt_u16(uint16_t value) {
  uint32_t result = 0;
  const uint32_t widened = value;
  for (int bit = 7; bit >= 0; --bit) {
    const uint32_t candidate = result + (1u << static_cast<uint32_t>(bit));
    if (candidate * candidate <= widened)
      result = candidate;
  }
  return static_cast<uint16_t>(result);
}

void verify(const g2_data::VectorPackRef &out,
  ...
  for (uint32_t vector = 0; vector < kVectors; ++vector) {
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
           ++element, ++offset) {
        const uint16_t expected = isqrt_u16(src[offset]);
...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: " << argv[0] << " SRC.bin OUT.bin REPETITIONS\n";
    return 2;
  }
  ...
  try {
    ...
    try {
      ...
      const ApuG2SqrtParams params{kSrcRow, kOutRow, repetitions, timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_sqrt", params);
    ...
    std::cerr << "APUg2 sqrt host failure: " << error.what() << '\n';
    return 1;
  }
}
