...
constexpr uint32_t kMatrixRow = 64;
constexpr uint32_t kXRow = 160;
constexpr uint32_t kAccumulatorRow = 192;
...
int main(int argc, char **argv) {
  if (argc != 8) {
    std::cerr << "usage: " << argv[0]
              << " MATRIX.bin X.bin ACC.bin TMP.bin OUT.bin"
  ...
  try {
    ...
    const auto accumulator_values = read_u16_file(argv[3], kPackElements);
    ...
    const gsi::VectorPack matrix(descriptor, kMatrixRow);
    ...
    g2_data::VectorPackRef matrix_ref(matrix, kGroups);
    ...
    fill_pack(matrix_ref, matrix_values);
    ...
    try {
      ...
      const ApuG2GemvParams params{
          kMatrixRow,      kXRow,   kAccumulatorRow, kTmpRow,
          kOutRow,         static_cast<uint32_t>(parsed_log),
          static_cast<uint32_t>(parsed_repetitions), timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_gemv", params);
      ...
      write_pack(argv[5], out_ref);
      const auto all_stop = Clock::now();

      std::cout << "device_product_ticks=" << timings.product_ticks << '\n'
      ...
      std::cout << "PASS gemv_physical_outputs=" << 2 * kPackElements
                << '\n';
      return 0;
    } catch (...) {
      if (timings_l5_live)
    ...
    std::cerr << "APUg2 GEMV host failure: " << error.what() << '\n';
    return 1;
  }
}
