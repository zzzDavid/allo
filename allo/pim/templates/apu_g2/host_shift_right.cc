...
void verify(const g2_data::VectorPackRef &out, const std::vector<uint16_t> &src,
            uint32_t shift) {
  uint64_t offset = 0;
  for (uint32_t vector = 0; vector < kVectors; ++vector) {
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
        ...
        const uint16_t expected =
            static_cast<uint16_t>(static_cast<uint32_t>(src[offset]) >> shift);
...
int main(int argc, char **argv) {
  if (argc != 5) {
    std::cerr << "usage: " << argv[0]
              << " SRC.bin OUT.bin SHIFT REPETITIONS\n";
    return 2;
  }

  try {
    const uint64_t parsed_shift = std::stoull(argv[3]);
    if (parsed_shift > 15)
      throw std::invalid_argument("shift must be in [0, 15]");
    const uint32_t shift = static_cast<uint32_t>(parsed_shift);
    ...
    try {
      ...
      const ApuG2ShiftRightParams params{kSrcRow, kOutRow, shift, repetitions,
                                         timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_shift_right", params);
      ...
      verify(out_ref, src_values, shift);
    ...
    std::cerr << "APUg2 shift_right host failure: " << error.what() << '\n';
    return 1;
  }
}
