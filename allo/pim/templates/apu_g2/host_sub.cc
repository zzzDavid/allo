    ...
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
        ...
        const uint16_t expected = static_cast<uint16_t>(
            static_cast<uint32_t>(lhs[offset]) -
            static_cast<uint32_t>(rhs[offset]));
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    try {
      ...
      const ApuG2SubParams params{kLhsRow, kRhsRow, kOutRow, repetitions,
                                  timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_sub", params);
    ...
    std::cerr << "APUg2 sub host failure: " << error.what() << '\n';
    return 1;
  }
}
