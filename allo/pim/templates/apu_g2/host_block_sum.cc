...
uint64_t offset_of(uint32_t vector, uint32_t group, uint32_t element) {
  return (uint64_t{vector} * kGroups + group) * kElementsPerGroup + element;
}

uint64_t verify(const g2_data::VectorPackRef &out,
                const std::vector<uint16_t> &src, uint32_t log_block_size) {
  const uint32_t block_size = 1u << log_block_size;
  uint64_t checked = 0;
  for (uint32_t vector = 0; vector < kVectors; ++vector) {
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
           element += block_size) {
        uint32_t sum = 0;
        for (uint32_t lane = 0; lane < block_size; ++lane)
          sum += src[offset_of(vector, group, element + lane)];
...
int main(int argc, char **argv) {
  if (argc != 5) {
    std::cerr << "usage: " << argv[0]
              << " SRC.bin OUT.bin LOG_BLOCK_SIZE REPETITIONS\n";
    return 2;
  }

  try {
    const uint64_t parsed_log = std::stoull(argv[3]);
    if (parsed_log > 8)
      throw std::invalid_argument("log block size must be in [0, 8]");
    const uint32_t log_block_size = static_cast<uint32_t>(parsed_log);
    ...
    const auto src_values = read_u16_file(argv[1]);
    ...
    const gsi::VectorPack src(descriptor, kSrcRow);
    const gsi::VectorPack out(descriptor, kOutRow);
    g2_data::VectorPackRef src_ref(src, kGroups);
    g2_data::VectorPackRef out_ref(out, kGroups);
    fill_ref(src_ref, src_values);
    ...
    try {
      ...
      const ApuG2BlockSumParams params{kSrcRow, kOutRow, log_block_size,
                                       repetitions, timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_block_sum", params);
      ...
      const uint64_t checked = verify(out_ref, src_values, log_block_size);
      write_u16_file(argv[2], out_ref);
    ...
    std::cerr << "APUg2 block_sum host failure: " << error.what() << '\n';
    return 1;
  }
}
