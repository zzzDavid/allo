...
int main(int argc, char **argv) {
  ...
  try {
    ...
    try {
      ...
      const ApuG2MulParams params{kLhsRow, kRhsRow, kOutRow, repetitions,
                                  timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_mul", params);
    ...
    std::cerr << "APUg2 mul host failure: " << error.what() << '\n';
    return 1;
  }
}
