...
void sub(const MmbVectors_seg0 &, const MmbVectors_seg1 &);
...
struct SubViews {
  ...
  explicit SubViews(const ApuG2SubParams &p)
...
inline void sub_loaded(const SubViews &v) {
  copy(v.lhs, v.lhs_mmb);
  copy(v.rhs, v.rhs_out_mmb);
  sub(v.lhs_mmb, v.rhs_out_mmb);
...
GSI_LIBRARY_ENTRY_POINT(tenon_apu_g2_u16_sub, uint64_t params_l5) {
  const auto *p = reinterpret_cast<const ApuG2SubParams *>(
      static_cast<uintptr_t>(params_l5));
  if (p == nullptr || p->timings_l5 == 0 || p->repetitions == 0)
    return GSI_STATUS_GENERAL_ERROR;

  const SubViews v(*p);
  ApuG2SubTimings timings{};
  ...
  timings.final_pipeline_ticks = time_fragment([&]() { sub_loaded(v); });

  auto *result = reinterpret_cast<ApuG2SubTimings *>(
