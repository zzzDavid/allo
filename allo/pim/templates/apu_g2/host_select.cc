...
constexpr uint32_t kTrueRow = 448;
constexpr uint32_t kFalseRow = 640;
...
void verify(const g2_data::VectorPackRef &out, const std::vector<uint16_t> &lhs,
            ...
            const std::vector<uint16_t> &true_values,
            const std::vector<uint16_t> &false_values) {
  uint64_t offset = 0;
  for (uint32_t vector = 0; vector < kVectors; ++vector) {
    for (uint32_t group = 0; group < kGroups; ++group) {
      for (uint32_t element = 0; element < kElementsPerGroup;
        ...
        const uint16_t expected =
            lhs[offset] < rhs[offset] ? true_values[offset] : false_values[offset];
...
int main(int argc, char **argv) {
  if (argc != 7) {
    std::cerr << "usage: " << argv[0]
              << " LHS.bin RHS.bin TRUE.bin FALSE.bin OUT.bin REPETITIONS\n";
    return 2;
  }

  try {
    const uint64_t parsed_repetitions = std::stoull(argv[6]);
    ...
    const auto true_values = read_u16_file(argv[3]);
    const auto false_values = read_u16_file(argv[4]);
    ...
    const gsi::VectorPack true_pack(descriptor, kTrueRow);
    const gsi::VectorPack false_pack(descriptor, kFalseRow);
    ...
    g2_data::VectorPackRef true_ref(true_pack, kGroups);
    g2_data::VectorPackRef false_ref(false_pack, kGroups);
    g2_data::VectorPackRef out_ref(out, kGroups);
    fill_ref(lhs_ref, lhs_values);
    fill_ref(rhs_ref, rhs_values);
    fill_ref(true_ref, true_values);
    fill_ref(false_ref, false_values);
    ...
    copy_to_l1(device, true_ref);
    copy_to_l1(device, false_ref);
    ...
    try {
      ...
      const ApuG2SelectParams params{kLhsRow,  kRhsRow, kTrueRow, kFalseRow,
                                     kOutRow, repetitions, timings_l5};
      const auto task_start = Clock::now();
      const int task_rc = device.run_task("tenon_apu_g2_u16_select_lt", params);
      ...
      verify(out_ref, lhs_values, rhs_values, true_values, false_values);
      write_u16_file(argv[5], out_ref);
    ...
    std::cerr << "APUg2 select-lt host failure: " << error.what() << '\n';
    return 1;
  }
}
