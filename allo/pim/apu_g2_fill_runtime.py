...
"""Hardware-only uint16 scalar-fill primitive for Gemini-II."""
...
from .apu_g2_recipe import build_apu_g2_u16_fill_recipe
...
def _validate_value(value) -> int:
    ...
    if not 0 <= value <= np.iinfo(np.uint16).max:
        raise ValueError("fill value must fit uint16")
    return value
...
def _parse_metrics(text: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    if f"PASS checked={np.prod(_SHAPE)}" not in text:
        raise RuntimeError("hardware host did not report its all-lane fill gate")
    return ticks, host


def run_apu_g2_u16_fill(value, *, repetitions=128) -> RunResult:
    """Fill one full ``(4, 65536)`` uint16 vector pack on the real card."""
    ...
    recipe = build_apu_g2_u16_fill_recipe(value)

    keep = os.environ.get(_KEEP_ENV) == "1"
    root = Path(tempfile.mkdtemp(prefix="tenon-apu-g2-fill-"))
    ...
    try:
        ...
        host_binary = host_build / "tenon_apu_g2_u16_fill"
