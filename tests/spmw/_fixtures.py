    ...
    def device():
        ...
        def pseudo_channel():
            ...
            def pim():
                ...
                # Samsung HBM-PIM moves. Per design 04 the cycle numbers
                # no longer live on the target tree -- they migrated to the
                # bound CostModel (spmw_cost_tables.SAMSUNG_FAITHFUL). The
                # target carries structure + emit only. JUMP is a synthetic
                # control op the cost model prices for the inner-K fold; no
                # codegen emits it.
                allo.move(
