...
def test_drain_emits_nop_not_bank_mov():
    ...
    for c in ctx.cmds:
        assert not (c.dst_ in ("EVEN_BANK", "ODD_BANK")
                    and any(s in ("GRF_A", "GRF_B")
...
def test_resolve_host_moves_builds_resolved_records():
    ...
    assert [r.move.name for r in resolved] == [
        "SCATTER_BANKS", "BCAST_GRF_A", "GATHER_BANKS"]
...
def test_provenance_crosscheck_does_not_change_cost():
    # The provenance helper must be side-effect-free on cost (no exception, no
    # mutation of the layout's other extra keys).
    from allo.spmw_cost_tables import _attribute_host_staging_provenance
    from allo.spmw_autoschedule import Placement
    pl = Placement(placements={}, extra={"stage_resident": True})
    pl.extra["host_moves"] = [
        ResolvedHostMove(scatter, object(), object(), "A"),
        ResolvedHostMove(broadcast, object(), object(), "x"),
        ResolvedHostMove(gather, object(), object(), "out"),
    ]
    # full coverage -> no warning, no raise
    _attribute_host_staging_provenance(pl, M=128, K=16)
    assert pl.extra["stage_resident"] is True
