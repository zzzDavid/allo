...
"""PolyBench syr2k on samsung_hbm_pim -- Tier-1 single-output cell (task 005).

Thin cell: build the target from `lib`, run the shared `syr2k` workload through
...
_RUN_CMD = (
    "python -m pytest tests/pim/samsung_hbm_pim/syr2k/test_syr2k_samsung_hbm_pim.py "
...
def test_syr2k_samsung_hbm_pim(request):
    ...
    # (samsung_hbm_pim/syr2k/workload.py, slice form) by run_cell; STAGES re-read
    # from it.
