...
"""PolyBench gemver on APU v1 -- Tier-1 REAL-DEVICE cell (task 007).

Thin cell: build the apu_v1 target from `lib`, run the shared `gemver` workload
...
_TARGET = "apu_v1"
_RUN_CMD = (
    "python -m pytest tests/pim/apu_v1/gemver/test_gemver_apu_v1.py "
    "-p no:cacheprovider -q"
)


@pytest.mark.apu_v1_device
def test_gemver_apu_v1(request, apu_v1_device_gate):
