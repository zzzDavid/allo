...
"""PolyBench symm on APU v1 -- Tier-2 triangular REAL-DEVICE cell (task 008).

Thin cell: build the apu_v1 target from `lib`, run the shared `symm` workload
...
_TARGET = "apu_v1"
_RUN_CMD = (
    "python -m pytest tests/pim/apu_v1/symm/test_symm_apu_v1.py "
    "-p no:cacheprovider -q"
)


@pytest.mark.apu_v1_device
def test_symm_apu_v1(request, apu_v1_device_gate):
