...
**Date:** 2026-03-04
**Status:** K-streaming optimization applied, gap reduced from 1.6x to 1.08x
**Branch:** minisa
...
|---|---|---|---|
| **Allo K-streaming (current)** | **1213** | 8 | Gr=AW=4, K-fused, no dividers |
...
**Current gap: 1.08x** (was 1.6x). K-streaming delivered a **1.48x speedup** (1792 -> 1213).
...
## Remaining Gap Analysis (1213 vs 1120 = 93 cycles)

The remaining 8% gap is likely from:

1. **No ping-pong buffering** (~50-70 cycles): Crossbar fill and NEST compute are
   sequential. The RTL double-buffers crossbar data, overlapping next tile's load
   with current tile's compute. With 8 tiles, this saves ~8-10 cycles/tile.
...
- RTL cosim: `test_figure7_cosim` — PASSED (1213 cycles, all outputs match)
