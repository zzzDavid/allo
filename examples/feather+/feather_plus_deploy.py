...
"""FEATHER+ DEPLOYABLE variant — iter70 w_chain weight cascade (Allo dataflow).

This is the routability-oriented port of the deployable HLS reference
(`feather_plus_16x16` commit beec78e / iter70, target ~30,446 cosim / 75,040 hw
cycles @ 250 MHz, routes clean on U280). Unlike the perf variant
(`feather_plus_perf.py`, a single monolithic II=1 ring_core that is fast but
congestion-7 unroutable), the deployable design is a DISTRIBUTED multi-process
dataflow that deliberately trades cycles for routability by de-densifying the
per-cycle operand fan-out with nearest-neighbour cascades.

This variant = the baseline `feather_plus.py` multi-process dataflow topology
with ONE structural change: the weight distribution is converted from the
baseline `w_broadcast` fan-out (per-cycle weight fan-out = AW) to the iter70
nearest-neighbour `w_chain` weight cascade (per-cycle fan-out = 1, the iter70
routability lever, +11,546 cosim accepted for clean routing). The MAC array,
BIRRD reduction, and accumulation are kept byte-identical to the baseline, so the
result is BIT-EXACT: the cascade only changes which cycle/port a column's weight
arrives on, never the values or MAC reduction order.

Architecture (dataflow kernels):
1. a_loader: Reads A from DRAM, column-streams to AW column heads
2. w_loader: Reads W from DRAM, emits all columns COLUMN-MAJOR onto w_chain[0]
3. w_relay[AW]: nearest-neighbour weight cascade — instance nj peels its own
   column to pe_w_in[*, nj] and forwards columns nj+1..AW-1 to w_chain[nj+1]
   (per-cycle weight fan-out AW -> 1; replaces the baseline w_broadcast)
...
The w_chain FIFO depth is 32 (the iter70 cvec_s 4->32 lesson) to cover the
cascade skew (early columns relay a long tail before later columns drain).
...
def get_feather_deploy_top(M, K, N, AW, AH, Ty, num_inst,
    ...
    2. w_loader(1): Reads W, emits columns column-major onto w_chain[0]
    3. w_relay[AW]: nearest-neighbour weight cascade -> pe_w_in[*, nj]
    ...
    def deploy_top(
        ...
        # Column input streams: a_loader -> PE row 0 (A)
        col_a_in: Stream[int32, AH][AW]
        # === DEPLOY w_chain weight cascade (iter70 routability lever) ===
        # The baseline broadcasts W to AW per-column FIFOs (per-cycle weight
        # fan-out = AW). iter70 collapses that fan-out to 1 with a
        # nearest-neighbour RELAY: w_loader emits every column's weight onto a
        # SINGLE chain head w_chain[0] (column-major per (op, nk, pe_row)); each
        # relay instance nj peels its OWN column's value to pe_w_in[*, nj] and
        # forwards the remaining columns to w_chain[nj+1]; w_sink drains the tail
        # w_chain[AW]. Per-cycle weight fan-out: AW -> 1. The VALUES and MAC
        # reduction order are unchanged (bit-exact) — only the delivery topology
        # (which cycle/port a column's weight arrives on) changes.
        # Depth 32 covers the cascade skew (early columns forward a long tail
        # before later columns drain) — the iter70 cvec_s 4->32 lesson.
        w_chain: Stream[int32, 32][AW + 1]
        # Per-(row,col) W streams: relay nj -> PE[row, nj]
        ...
        def w_loader(
            ...
            """Reads W from DRAM, emits onto the w_chain head (column-major).

            DEPLOY change vs baseline: instead of fanning W out to AW per-column
            FIFOs, the loader writes a SINGLE chain head w_chain[0]. For each
            (op, nk, pe_row) it emits the columns in order nj=0..AW-1; the relay
            cascade downstream peels each column to its own PE. Per-cycle weight
            fan-out from the loader is 1 FIFO (vs AW) — the iter70 routability
            lever. Values are identical to the baseline (same B reads / zp).
            """
                ...
                for inner in range(n_inner):
                    ...
                    for nk in range(AH):
                        for pe_row in range(AH):
                            # emit all AW columns onto the single chain head,
                            # column-major (nj ascending). Compile-time unrolled.
                            with allo.meta_for(AW) as nj:
                                k_idx: int32 = k_start_tile + nk + (nj >> log2_Gr) * AH
        ...
        def w_relay():
            """Nearest-neighbour weight relay (iter70 w_chain cascade).

            Instance nj reads its share of the chain (columns nj..AW-1, in order,
            per (op, nk, pe_row)), keeps its OWN column's value -> pe_w_in[*, nj],
            and FORWARDS the remaining columns nj+1..AW-1 to w_chain[nj+1]. Because
            nj = get_pid() is a compile-time constant per instance, the forward
            count (AW-1-nj) is resolved at compile time by the meta_if guard — no
            runtime-variable loop bound, and per-cycle weight fan-out is 1.
            """
            nj = df.get_pid()
            for _op in range(total_ops):
                for nk in range(AH):
                    # pe_row must be a meta_for: it indexes the pe_w_in Stream
                    # array (Stream indices must be compile-time). The unroll
                    # order (pe_row 0..AH-1) matches the loader's emission order.
                    with allo.meta_for(AH) as pe_row:
                        # first value on this instance's chain head is its own
                        # column's weight; the rest belong to columns > nj.
                        w_val: int32 = w_chain[nj].get()
                        pe_w_in[pe_row, nj].put(w_val)
                        # forward the downstream columns (compile-time count).
                        # The last instance (nj == AW-1) has no downstream cols,
                        # so its meta_if body is empty — w_chain[AW] is declared
                        # for index symmetry but never written/read (same idiom
                        # as the baseline's unused pe_a_down[AH-1, *] tail).
                        with allo.meta_if(nj < AW - 1):
                            with allo.meta_for(AW) as relay_col:
                                with allo.meta_if(relay_col > nj):
                                    fwd_val: int32 = w_chain[nj].get()
                                    w_chain[nj + 1].put(fwd_val)
...
def build_feather_deploy_simulator(M, K, N, AW, AH, Ty, num_inst,
...
def schedule_deploy_hls(s, K, N, AH, AW):
    ...
    # w_loader: tile loop pipelined; emits weights column-major onto w_chain[0].
    # w_relay[AW]: nearest-neighbour cascade -> per-(row,col) pe_w_in FIFOs
    # (replaces the baseline w_broadcast fan-out — the iter70 routability lever).
    s.pipeline("w_loader_0:tile")
    # Partition C dim=2 (N-dimension) — Complete for parallel column writes.
    s.partition("deploy_top:C", dim=2, partition_type=Partition.Complete)
...
def build_feather_deploy_hls(M, K, N, AW, AH, Ty, num_inst,
    ...
    s = df.customize(top)
    schedule_deploy_hls(s, int(K), int(N), int(AH), int(AW))
