...
# Name of the generated HLS top function. The baseline region is
# `full_matrix_top`; the variant runners override this module-level value
# (test_trace_perf.py -> "perf_top", test_trace_deploy.py -> "deploy_top") so the
# cosim flow (set_top, the tb.cpp call, and the *_cosim.rpt filename) targets the
# correct generated top. This is harness wiring only — the Allo source / generated
# kernel.cpp are untouched.
TOP_NAME = "full_matrix_top"
...
def _generate_cosim_testbench(project_dir, trace_info, A, B, instructions,
    ...
    with open(tb_path, "w") as f:
        ...
        f.write(f"  {TOP_NAME}(A_pe, inst_pe, loader_m_start, "
